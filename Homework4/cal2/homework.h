...
void cal()
{
	while (1)
	{
		...
		sr:
			...
			if (!Error(example))
			{
				TransmitExpression(example, postexp);
				printf("后缀表达式为：%s\n", postexp);
				printf("表达式结果为：%s=", example);
				printf("%g\n", EvaluateExpression(postexp));
				result = EvaluateExpression(postexp);
